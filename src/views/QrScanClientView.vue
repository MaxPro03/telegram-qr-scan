...
const sendPostRequest = async (id) => {
    ...
    if (route.params.orderId != id) {
        isSuccessMessage.value = false
        errorMessage.value = 'Номер заказа не совпадает'
        openModal()
        return
    }
    ...
    try {
        const response = await fetch(
            `https://admin.gogomarket.uz/api/v2/telegram-bot/confirm-order-delivery?telegram_user_id=${tg?.initDataUnsafe?.user?.id}&order_id=${id}`,
            {
                method: 'POST',
                headers: {
